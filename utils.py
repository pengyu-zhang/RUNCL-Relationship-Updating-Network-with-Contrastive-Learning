...
def load_data(path, dataset):
    ...
    if dataset in ['cora', 'citeseer', 'pubmed']:
        ...
        print(f"Number of edges: {sum([len(graph[node]) for node in graph]) / 2}")
        print(f"Number of features: {x.shape[1]}")
        print(f"Number of test instances: {len(ty)}")
...
###############################################
        ...
        # 获取节点度并进行排序
        adj_list = graph
        test_degrees_nodes = [(node, len(adj_list[node])) for node in test_idx_reorder]
        test_degrees_nodes.sort(key=lambda x: x[1])
        mid_idx = len(test_degrees_nodes) // 2
        low_degree_nodes = [node for node, degree in test_degrees_nodes[:mid_idx]]
        high_degree_nodes = [node for node, degree in test_degrees_nodes[mid_idx:]]

        # 根据需要选择度高或度低的节点作为测试集
        chosen_test_nodes = high_degree_nodes  # 或者 high low_degree_nodes

        # 更新 test_idx_reorder 和 test_idx_range
        test_idx_reorder = chosen_test_nodes
        test_idx_range = np.sort(test_idx_reorder)
        # 打印测试集节点数量
        print(f"Number of test instances: {len(test_idx_range)}")
...
##########################################
        # 更新 label 和 feature 以仅包含 chosen_test_idx 中的节点
        feature[test_idx_reorder, :] = feature[test_idx_range, :]
        label[test_idx_reorder] = label[test_idx_range]
##########################################
        ...
        for j in range(num_class):
            idx_train.extend([i for i,x in enumerate(label) if x==j][:15])
