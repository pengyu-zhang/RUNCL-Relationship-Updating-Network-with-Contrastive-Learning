...
parser.add_argument('--base', type=str, default='gcn', choices=['gcn', 'sgc', 'gat', 'appnp', 'sage', 'grace'])
...
parser.add_argument('--act_fn', type=str, default='relu', choices=['relu', 'leaky_relu', 'elu'])
...
parser.add_argument('--iter', type=int, default=20, help='number of iterations to train the GEN')
...
base_model_args = {"num_feature": data.num_feature, "num_class": data.num_class, "hidden_size": args.hidden,
                   "dropout": args.dropout, "activation": args.activation}
if args.base == 'gcn':
    base_model = GCN(**base_model_args)
elif args.base == 'grace':
    base_model = GCN1(**base_model_args)
# elif args.base == 'sgc':
