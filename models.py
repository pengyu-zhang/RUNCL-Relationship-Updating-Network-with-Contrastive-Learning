...
from aug import aug
from utils import accuracy, accuracy1, sparse_mx_to_sparse_tensor, prob_to_adj
import wandb
import argparse
...
class GEN:
    ...
    def fit(self, data):
        ...
        for iter in range(args.iter):
            start = time.time()
            self.train_base_model(data, adj, iter, adj_re)
    ...
    def train_base_model(self, data, adj, iter, adj_re):
        best_acc_val = 0
        act_fn = ({"relu": nn.ReLU(), "prelu": nn.PReLU()})[self.args.act_fn]
        model1 = Grace(in_dim=1433, hid_dim=128, out_dim=128, num_layers=2, act_fn=act_fn, temp=0.4)
        ...
        for epoch in range(self.args.epoch):
            ...
            optimizer.zero_grad()
            # grace
            # graph1, feat1 = aug(adj_re, data.x, self.args.dfr1, self.args.der1)
            # graph2, feat2 = aug(adj_re, data.x, self.args.dfr2, self.args.der2)
            # graph3, feat3 = aug(adj, data.x, self.args.dfr1, self.args.der1)
            # graph4, feat4 = aug(adj, data.x, self.args.dfr2, self.args.der2)
            # graph1 = graph1.to(self.device)
            # graph2 = graph2.to(self.device)
            # graph3 = graph3.to(self.device)
            # graph4 = graph4.to(self.device)
            # feat1 = feat1.to(self.device)
            # feat2 = feat2.to(self.device)
            # feat3 = feat3.to(self.device)
            # feat4 = feat4.to(self.device)
            # grace over
            hidden_output, output = self.base_model(data.x, adj)
            loss_train = F.nll_loss(output[data.mask_train], data.y[data.mask_train])# + model1(graph1, graph2, feat1, feat2) + model1(graph3, graph4, feat3, feat4)
            ...
            if self.args.debug:
                if epoch % 1 == 0:
                    # wandb.log(
                    #     {"loss_train": loss_train.item(), "acc_train": acc_train.item(), "loss_val": loss_val.item(),
    ...
    def test(self, data):
        ...
        acc_test = accuracy1(output[data.mask_test], data.y[data.mask_test])
        acc_val = accuracy(output[data.mask_val], data.y[data.mask_val])
